...
def parse(arg):
    curly__braces = re.search(r"\{(.*?)\}", arg)
    b_brackets = re.search(r"\[(.*?)\]", arg)
    if curly__braces is None:
        if b_brackets is None:
            return [i.strip(",") for i in split(arg)]
        else:
            lexer = split(arg[:b_brackets.span()[0]])
            retl = [i.strip(",") for i in lexer]
            retl.append(b_brackets.group())
            return retl
    else:
        lex = split(arg[:curly__braces.span()[0]])
        rex = [i.strip(",") for i in lex]
        rex.append(curly__braces.group())
        return rex
